...
function diff(level){
  const t=(level-1)/29; // 0..1 dla 30 poziomów
  return {
    wallDensity: 0.24 + t*0.26,          // coraz ciaśniej
    carveSteps: 220 + Math.floor(t*180), // dłuższe węże (labirynt)
...
// Kod poziomu 7-znakowy
function levelCodeFor(level){
  // prosty PRNG oparty o level
...
const boka={x:48,y:48,r:BOKA_R_BASE,hp:HP_MAX,poison:0,
  ...
  growT:0, growSpeedMul:1.0, // przy muchomorze
...
function genMap(level){
  ...
  // labirynt: węże dłuższe dla wyższego poziomu
...
// Bobek – plan budowy 5 sieci przez ~60s (ok. 10s/sieć), biega między punktami
function triggerBobek(){
  if(bobek.active) return;
  if(bobek.callsLeft<=0) return;
  bobek.callsLeft--;
  sfxBobek.currentTime=0; sfxBobek.play();
  // wyznacz 5 punktów po stronie domku (losowo jak pickupy)
...
function planWebSpotsNearHouse(n){
  // znajdź bok planszy, na którym leży środek domku
...
function updateBobek(dt){
  ...
  if(bobek.building){
    ...
    // po 10 sekundach sieć aktywna
    if(bobek.building.time>=10){
      const t= bobek.building.trap;
      t.active=true; t.buildProgress=1;
      bobek.building=null;
      // jeśli skończył wszystkie punkty – wraca do jaskini
      if(bobek.plan.length===0){ /* kierunek do jaskini */ }
    }
  } else {
    // jeśli są jeszcze punkty, biegnij do następnego
    if(bobek.plan.length>0){
      ...
      if(d<12){
        // zacznij budowę: dodaj „nieaktywną” pułapkę z rosnącym progresem
...
function updateTraps(dt){
  for(const t of traps){
    // palenie sieci przez Włóczykija: burning 0..15s
    if(t.burning!=null){
      t.burning += dt;
      if(t.burning>=15){ t.active=false; t.burning=null; }
    }
...
function moveCircle(obj,vx,vy,rad){
  ...
  for(let rr2=r-1;rr2<=r+1;rr2++) for(let cc=c-1;cc<=c+1;cc++){
    ...
    if(grid[rr2][cc]===0 || grid[rr2][cc]===2) continue; // dom nie blokuje
...
function update(now){
  ...
  // Buka – wielkość i prędkość (po grzybku maleje stopniowo, prędkość teraz 3x podczas efektu)
  if(boka.growT>0){
    boka.growT=Math.max(0,boka.growT-dt);
    boka.growSpeedMul=3.0; // było 1.5x, teraz 3x
    // płynne zmniejszanie promienia z 4x do 1x wraz z upływem growT (bazowo 15s)
  ...
  // Hunter – AI: ratowanie ma priorytet. Po drodze spala sieci (15s/sieć)
  if(hunter.alive){
    if(hunter.frozen>0) hunter.frozen-=dt;
    else{
      const trapped = mumins.find(m=>m.alive && m.trapped);
      if(trapped){
        // jeśli na trasie jest aktywna sieć blisko linii do celu – spal ją najpierw
        let nextTrap = nearestBlockingTrap(hunter, trapped);
        if(nextTrap){
          // „podpal” jeśli nie płonie i nie spalona
          if(nextTrap.active && nextTrap.burning==null){
            // idź do sieci
            goTowards(hunter, nextTrap.x, nextTrap.y, d.hunterSpeed);
            if(dist(hunter.x,hunter.y,nextTrap.x,nextTrap.y)<14) nextTrap.burning=0;
          }else{
            goTowards(hunter, trapped.x, trapped.y, d.hunterSpeed);
            if(dist(hunter.x,hunter.y,trapped.x,trapped.y)<14){ trapped.trapped=false; trapped.frozen=0; }
          }
        } else {
          goTowards(hunter, trapped.x, trapped.y, d.hunterSpeed);
          if(dist(hunter.x,hunter.y,trapped.x,trapped.y)<14){ trapped.trapped=false; trapped.frozen=0; }
        ...
        // brak uwięzionych – jeśli są aktywne sieci, niszcz je; w przeciwnym razie atakuj Bukę
        const activeTrap = traps.find(t=>t.active);
        if(activeTrap){
          goTowards(hunter, activeTrap.x, activeTrap.y, d.hunterSpeed);
          if(dist(hunter.x,hunter.y,activeTrap.x,activeTrap.y)<14 && activeTrap.burning==null) activeTrap.burning=0;
        } else {
          // atak Buki
  ...
  // Pioruny Ctrl – kolizje z Muminkami/Włóczykijem/Hatifnatami
  for(let i=iceBolts.length-1;i>=0;i--){
    ...
    for(const h of hattis){
      if(Math.hypot(b.x-h.x,b.y-h.y)<h.r* h.scale + 6){
        h.scale*=1.10; // rośnie o 10% po trafieniu
        b.ttl=0; break;
      }
    }
  }
  ...
  for(const m of mushrooms){
    if(m.alive===false){
      ...
      if(Math.hypot(m.x-boka.x,m.y-boka.y)<boka.r+m.r){
        ...
        boka.growT=15; // czas trwania efektu
        // prędkość ustawiana w update()
      }
    }
  }
  ...
  for(let i=hattis.length-1;i>=0;i--){
    const h=hattis[i];
    const sp=BOKA_BASE_SPEED*CELL/10 * (boka.growSpeedMul); // Buka szybciej => oni trzymają tempo zmienne
    if(h.flee){
...
function nearestBlockingTrap(src, dst){
  // najbliższa aktywna sieć w promieniu 80px od linii
  let best=null, bestD=1e9;
  for(const t of traps){
    if(!t.active) continue;
    const d=pointLineDistance({x:t.x,y:t.y},{x:src.x,y:src.y},{x:dst.x,y:dst.y});
    if(d<80){
      const dd=dist(src.x,src.y,t.x,t.y);
...
function draw(){
  ...
  // pułapki – pajęczyny, animacja budowy i palenia
  for(const t of traps){
    // build progress 0..1
  ...
  // Hatifnatowie (zwiększający się rozmiar)
  for(const h of hattis){
...
function startGame(reset=false){
  if(reset){ score=0; boka.ammoAlt=100; boka.ammoCloud=5; boka.hp=HP_MAX; }
  resetLevel();
