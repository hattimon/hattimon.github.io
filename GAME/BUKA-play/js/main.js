...
// Trudność/skalowanie na 30 poziomów
function diff(level){
  const t=(level-1)/29;
  return {
...
musicBtn.addEventListener('click',()=>{
  ...
  if(!musicOn){ musicBg.pause(); musicPanic.pause(); } else { if(panic){ musicPanic.play(); } else { musicBg.play(); } }
});
...
startBtn.addEventListener('click',()=>{
  ...
  if(!L){
    ...
    if(desc && !desc.dataset.errShown){
      ...
      desc.appendChild(warn); desc.dataset.errShown='1';
    }
    isBooting=false;
    return;
  }
...
function safeStart(){
  // Startujemy po następnej klatce, by DOM zdążył ukryć overlay
  overlay.hidden=true;
  setTimeout(()=>{ startGame(true); isBooting=false; }, 0);
}
...
// Bobek – budowanie 5 sieci po 10s/sieć, max 3 wywołania
...
// Pułapki i palenie przez Włóczykija
...
function update(now){
  ...
  // Buka – muchomor: prędkość x3 i powolne zmniejszanie
  if(boka.growT>0){
  ...
  // Włóczykij – ratowanie/palenie sieci/przeciwnik
  if(hunter.alive){
...
function draw(){
  ...
  if(weather==='Noc'){ ctx.fillStyle='rgba(0,0,30,0.25)'; ctx.fillRect(0,0,canvas.width,canvas.height); }
    ...
    for(let i=0;i<50;i++){ const x=(performance.now()/10 + i*50)%canvas.width; const y=(i*37 + performance.now()/3)%canvas.height; ctx.beginPath(); ctx.moveTo(x,y); ctx.lineTo(x+4,y+10); ctx.stroke(); }
