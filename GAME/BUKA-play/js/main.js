(() => {
let booted=false;           // czy gra wystartowała
let starting=false;         // anty-dwuklik
...
function shade(hex, f){ const c=hex.replace('#',''); const r=parseInt(c.substr(0,2),16), g=parseInt(c.substr(2,2),16), b=parseInt(c.substr(4,2),16); const nr=Math.round(r*f), ng=Math.round(g*f), nb=Math.round(b*f); return `rgb(${nr},${ng},${nb})`; }
...
// START – klik przycisku
startBtn.addEventListener('click', tryStart);
// START – Enter w polu kodu
codeInput.addEventListener('keydown', (e)=>{
  if(e.key==='Enter'){ e.preventDefault(); tryStart(); }
});
// Czyść komunikat o złym kodzie przy zmianie
codeInput.addEventListener('input', ()=>{
  const d=overlay.querySelector('.desc');
  if(d && d.dataset.errShown){
    const last=d.querySelector('.pix[style*="ffb3b3"]');
    if(last) last.remove();
    delete d.dataset.errShown;
  }
});
...
function tryStart(){
  ...
  let L=1;
  if(code.length>0){
    const found=levelFromCode(code);
    if(!found){
      const d=overlay.querySelector('.desc');
      if(d && !d.dataset.errShown){
        const warn=document.createElement('div');
        warn.className='pix'; warn.style.marginTop='8px'; warn.style.color='#ffb3b3';
        warn.textContent='Zły kod poziomu. Zostaw puste, aby rozpocząć od poziomu 1.';
        d.appendChild(warn); d.dataset.errShown='1';
      }
      starting=false;
      return;
    }
  ...
  level=L;
  startGame(true);         // 1) przygotowanie
  overlay.hidden=true;     // 2) schowanie okna
  booted=true;
  starting=false;
}
...
function update(now){
  ...
  // Buka – muchomor: x3 i malejący rozmiar
  if(boka.growT>0){
  ...
  if(!boka.cloudActive && boka.rooted<=0){
    ...
    const moved=moveCircle(boka, ax*sp*dt, ay*sp*dt, boka.r); boka.x=moved.x; boka.y=moved.y;
  }
  ...
  if(aliveCount===0){
    stopAllMusic();              // zatrzymaj panic/tło
...
function draw(){
  ...
  for(const t of traps){
    ...
    ctx.save(); ctx.globalAlpha=0.4+0.6*build;
    const R=12*build;
    ...
    for(let r=4;r<=R;r+=4){ ctx.beginPath(); ctx.arc(t.x,t.y,r,0,Math.PI*2); ctx.stroke(); }
    ctx.restore();
  }
  ...
  // ogniki
  projs.forEach(p=>{
    const g=ctx.createRadialGradient(p.x,p.y,0.5,p.x,p.y,6);
    g.addColorStop(0,'rgba(255,240,160,1)');
    g.addColorStop(0.6,'rgba(255,140,80,0.9)');
  ...
  if(boka.cloudActive){
    ...
    grd.addColorStop(0,'rgba(0,0,0,0.6)'); grd.addColorStop(1,'rgba(0,0,0,0)'); ctx.fillStyle=grd;
    ctx.beginPath(); ctx.arc(boka.x,boka.y,Math.max(20,radius),0,Math.PI*2); ctx.fill();
  }
  ...
  for(const h of hattis){
    ...
    ctx.fillStyle='rgba(255,255,200,0.95)'; ctx.beginPath(); ctx.ellipse(0,0,6,12,0,0,Math.PI*2); ctx.fill();
    ctx.shadowBlur=0;
...
function startGame(reset=false){
  ...
  resetLevel();
  pauseOv.hidden=true; paused=false;
