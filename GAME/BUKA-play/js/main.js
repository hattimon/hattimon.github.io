...
function diff(level){
  const t=(level-1)/29; // 30 poziomów
  return {
...
// POPRAWIONY handler Start: pusty -> poziom 1; zły kod -> komunikat; dobry -> start poziomu z kodu
...
// Bobek – buduje 5 sieci w ~60s (ok. 10s/sieć), biega między punktami; max 3 wywołania/runda
...
function update(now){
  ...
  // Buka – efekt muchomora: prędkość x3 i powolne zmniejszanie
  if(boka.growT>0){
  ...
  for(const m of mushrooms){
    if(m.alive===false){
      ...
      if(Math.hypot(m.x-boka.x,m.y-boka.y)<boka.r+m.r){
        ...
        boka.growT=15; // prędkość x3 i powolne zmniejszanie
      }
    }
  }
