...
// Skala trudności 30 poziomów (zachowanie z 10 poziomów jako baza)
function diff(level){
  const t=(level-1)/29;
  return {
...
function stopAllMusic(){
  musicBg.pause(); musicPanic.pause(); sfxWin.pause();
}
function startBackground(){
  if(!musicOn) return;
  stopAllMusic();
  musicBg.currentTime=0; musicBg.loop=true; musicBg.play();
}
function startPanic(){
  if(!musicOn) return;
  musicBg.pause();
  musicPanic.currentTime=0; musicPanic.loop=false; musicPanic.play();
}
function startWinLoop(){
  stopAllMusic();
  sfxWin.currentTime=0; sfxWin.loop=true; sfxWin.play();
}
...
// START – wzorowane na działającej wersji (najpierw resetLevel, potem schowanie overlay i start pętli)
...
// Bobek – 5 sieci, ~10 s/sieć, max 3 wywołania
...
function update(now){
  ...
  // Buka – muchomor: x3 i zanikanie rozmiaru
  if(boka.growT>0){
    boka.growT=Math.max(0,boka.growT-dt);
    boka.growSpeedMul=3.0;
    const t=boka.growT/15; const scale = 1 + (4-1)*t;
  ...
  if(aliveCount===0){
    // Zatrzymaj panic i tło, potem natychmiast wystartuj tło następnego poziomu po startGame
    stopAllMusic();
...
// Final (wygrana gry) – tylko buka-win.mp3 w pętli
function finalWin(){
  playing=false;
  startWinLoop();
...
// Start gry – identyczna kolejność jak w działającej bazie
function startGame(reset=false){
  ...
  playing=true;
  if(!panic) startBackground(); else startPanic(); // bezpieczeństwo
  requestAnimationFrame(update);
}
