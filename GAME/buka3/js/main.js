...
// Audio zasady:
// - musicBg: loop zawsze, o ile muzyka ON.
// - pozostałe: play() jednorazowo na eventach.
let musicOn=true;
...
function resetLevel(){
  ...
  boka.x=48; boka.y=48; boka.hp=HP_MAX; boka.poison=0; boka.cloud=0; boka.cloudActive=false; boka.rooted=0; boka.lookX=1; boka.lookY=0; boka.growT=0; boka.globalFreeze=0; boka.chargeT=0; boka.charging=false;
...
function triggerBobek(){
  ...
  sfxBobek.currentTime=0; sfxBobek.play(); // jednorazowo
  bobek.active=true; bobek.x=cave.x; bobek.y=cave.y; bobek.dir=Math.random()*Math.PI*2; bobek.trapsLeft=5;
}
function updateBobek(dt){
  if(!bobek.active) return;
  const sp= bobek.trapsLeft>0 ? bobek.speed : bobek.speed*0.8;
  const moved=moveCircle({x:bobek.x,y:bobek.y}, Math.cos(bobek.dir)*sp, Math.sin(bobek.dir)*sp, bobek.r);
  bobek.x=moved.x; bobek.y=moved.y;
  if(Math.random()<0.02) bobek.dir=Math.random()*Math.PI*2;
  if(bobek.trapsLeft>0 && Math.random()<0.02){
    // w okolicy domku
    const spots=[];
    for(let r=0;r<ROWS;r++) for(let c=0;c<COLS;c++) if(grid[r][c]===2){ spots.push({x:c*CELL+CELL/2, y:r*CELL+CELL/2}); }
    if(spots.length){
      const s=spots[(Math.random()*spots.length)|0];
      traps.push({x:s.x+((Math.random()*26)|0)-13, y:s.y+((Math.random()*26)|0)-13, r:10, active:true});
      bobek.trapsLeft--;
    }
  }
  if(bobek.trapsLeft<=0){
    const dx=cave.x-bobek.x, dy=cave.y-bobek.y, d=Math.hypot(dx,dy);
    if(d<12){ bobek.active=false; } else { bobek.dir=Math.atan2(dy,dx); }
  }
}
...
// Hatifnatowie i muzyka PANIC (jednorazowo)
...
function update(now){
  ...
  if(hunter.alive && hunter.frozen<=0){
    ...
    if(trapped){
      ...
      if(dH>1){ const ux=dx/dH, uy=dy/dH; const moved=moveCircle(hunter, ux*d.hunterSpeed, uy*d.hunterSpeed, HUNTER_R); hunter.x=moved.x; hunter.y=moved.y; }
  ...
  for(const m of mumins){
    ...
    if(Math.hypot(m.x-boka.x,m.y-boka.y) < boka.r+m.r && !inHouse(m.x,m.y)){
      ...
      sfxDeathMumin.currentTime=0; sfxDeathMumin.play(); // jednorazowo
  ...
  // nagrody z chmurek strachu/niebieskich
...
function updatePickups(arr, kind){
  for(let i=0;i<arr.length;i++){
    ...
    if(p.alive && Math.hypot(p.x-boka.x,p.y-boka.y)<boka.r+p.r){
      ...
      p.timer-=1; // z grubsza 1s na wywołań pętli – uproszczenie
      if(p.timer<=0){
        for(let t=0;t<100;t++){
          const c=1+((Math.random()*(COLS-2))|0), r=1+((Math.random()*(ROWS-2))|0);
          if(grid[r][c]!==0) continue;
...
function draw(){
  ...
  for(let r=0;r<ROWS;r++) for(let c=0;c<COLS;c++){
    ...
    if(t===2){
      ctx.strokeStyle='rgba(200,255,220,0.25)'; ctx.strokeRect(x+3,y+3,w-6,h-6);
    }
  }
  ...
  for(const t of traps){
    ...
    ctx.beginPath(); ctx.rect(t.x-8,t.y-8,16,16); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(t.x-8,t.y-8); ctx.lineTo(t.x+8,t.y+8);
    ctx.moveTo(t.x+8,t.y-8); ctx.lineTo(t.x-8,t.y+8); ctx.stroke();
  }
  ...
  // globalFog/panicDim kontrolowane przez DOM/CSS
...
function finalWin(){
  ...
  sfxWin.currentTime=0; sfxWin.play(); // jednorazowo
...
function startGame(reset=false){
  ...
  playing=true;
  if(musicOn){ musicBg.currentTime=0; musicBg.play(); } // loop wg atrybutu w HTML
  requestAnimationFrame(update);
}
function themeFor(level){ return THEMES[(level-1)%THEMES.length]; }
